...
export const api = async (p: string, o?: RequestInit) => {
  const res = await fetch(p, { ...o, headers: { 'Content-Type':'application/json', ...(getToken() ? { Authorization:`Bearer ${getToken()}` } : {}), ...(o?.headers ?? {}) } });
