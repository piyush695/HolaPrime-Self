...
export async function integrationsHubRoutes(app: FastifyInstance) {
  ...
  // GET all integrations (with masked creds)
  app.get('/', async (_req, reply) => {
    ...
    const safe = (rows as any[]).map(r => {
      ...
      for (const [k, v] of Object.entries(creds)) {
        masked[k] = maskValue(k, String(v ?? ''));
      }
      return { service: r.service, label: r.label, is_active: r.is_active,
        last_tested: r.last_tested, test_result: r.test_result,
        updated_at: r.updated_at, credentials: masked };
    });
